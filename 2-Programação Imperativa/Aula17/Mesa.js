...
let ranking = todosLucro.sort((a, b) => {
  if (a.lucro < b.lucro) {
    return 1;
  }
  if (a.lucro > b.lucro) {
...
function trocaValor(x, y, z) {
  let itemEspecifico = x.filter((item) => { return item.nome == y });

  let alterarPreco = itemEspecifico.map((item) => { return item.preco = z });
  return alterarPreco;
}

//console.log(trocaValor(dados,"Cenoura",25));
//console.log(dados);
